...
class MeterEntity:
    ...
    def loadData(self, dat):
        ...
        self.addData(sorted(dat2, key= lambda date: date[0]))

    def addData(self, dat):
        meter = MeterData.objects.all().filter(meter_id=self.name, date__gte=dat[0][0], date__lte=dat[-1][0]).delete()
        forSave = []
        for row in dat:
            forSave.append(MeterData(date=row[0], value=row[1], meter_id=self.name))
        MeterData.objects.bulk_create(forSave)
